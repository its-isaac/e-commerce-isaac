...
import { deleteProductCartApi, decreaseProductCartApi, increaseProductCartApi } from "../../api/cart";
import colors from "../../styles/colors";

export default function Product(props) {
  const { product, setReloadCart, quantity } = props;
      ...
      <View style={styles.info}>
        <View>
          ...
          <View style={styles.prices}>
            <Text style={styles.currentPrice}> $ {calcPrice(product.price, product.discount)}
            </Text>
          </View> 
          {product.discount && (
            <View style={styles.containerDiscount}> 
              <Text style={styles.discountText}>Ahorras:</Text>
              <Text> $ {((product.price * product.discount) / 100).toFixed(2)} ({product.discount}%)
              </Text> 
            </View>
          )} 
        ...
        <View style={styles.btnsContainer}>
          ...
          <Button color="#000000" mode="contained" onPress={deleteProductCart}>
            Eliminar
          </Button>
        </View>
      </View>
